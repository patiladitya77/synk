import { ResizeShapeCommand } from "../ResizeShapeCommand";
import { Shape } from "../../types";
import { Socket } from "socket.io-client";
import React from "react";
import * as fc from "fast-check";
import {
  arbShape,
  arbRectangle,
  arbOval,
  arbDimension,
} from "../../test-utils/generators";

// Mock Socket.IO
interface MockSocket {
  emit: jest.Mock;
  on: jest.Mock;
  once: jest.Mock;
  off: jest.Mock;
}

function createMockSocket(): MockSocket {
  return {
    emit: jest.fn(),
    on: jest.fn(),
    once: jest.fn(),
    off: jest.fn(),
  };
}

function createMockShapesRef<T>(initialShapes: T[]): React.RefObject<T[]> {
  return {
    current: [...initialShapes],
  };
}

describe("ResizeShapeCommand", () => {
  let mockSocket: MockSocket;
  let shapesRef: React.RefObject<Shape[]>;
  let boardId: string;

  beforeEach(() => {
    mockSocket = createMockSocket();
    shapesRef = createMockShapesRef([]);
    boardId = "test-board-id";
  });

  describe("execute", () => {
    describe("Rectangle shapes", () => {
      it("should update rectangle dimensions in shapes array", () => {
        const oldRectangle: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 200,
          width: 80,
          height: 60,
        };

        const newRectangle: Extract<Shape, { type: "rect" }> = {
          ...oldRectangle,
          width: 120,
          height: 90,
        };

        shapesRef.current = [oldRectangle];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldRectangle,
          newRectangle,
        );

        command.execute();

        expect(shapesRef.current).toHaveLength(1);
        expect(shapesRef.current[0].type).toBe("rect");
        const resizedShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "rect" }
        >;
        expect(resizedShape.width).toBe(120);
        expect(resizedShape.height).toBe(90);
      });

      it("should emit updateShape event with correct payload for rectangle", () => {
        const oldRectangle: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 200,
          width: 80,
          height: 60,
        };

        const newRectangle: Extract<Shape, { type: "rect" }> = {
          ...oldRectangle,
          width: 120,
          height: 90,
        };

        shapesRef.current = [oldRectangle];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldRectangle,
          newRectangle,
        );

        command.execute();

        expect(mockSocket.emit).toHaveBeenCalledWith("updateShape", {
          boardId: boardId,
          shape: expect.objectContaining({
            id: "rect-1",
            type: "rect",
            width: 120,
            height: 90,
          }),
        });
      });

      it("should preserve rectangle position (x, y)", () => {
        const oldRectangle: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 200,
          width: 80,
          height: 60,
        };

        const newRectangle: Extract<Shape, { type: "rect" }> = {
          ...oldRectangle,
          width: 150,
          height: 120,
        };

        shapesRef.current = [oldRectangle];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldRectangle,
          newRectangle,
        );

        command.execute();

        const resizedShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "rect" }
        >;
        expect(resizedShape.x).toBe(100);
        expect(resizedShape.y).toBe(200);
      });

      it("should preserve rectangle type and id", () => {
        const oldRectangle: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 200,
          width: 80,
          height: 60,
        };

        const newRectangle: Extract<Shape, { type: "rect" }> = {
          ...oldRectangle,
          width: 100,
          height: 80,
        };

        shapesRef.current = [oldRectangle];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldRectangle,
          newRectangle,
        );

        command.execute();

        const resizedShape = shapesRef.current[0];
        expect(resizedShape.id).toBe("rect-1");
        expect(resizedShape.type).toBe("rect");
      });

      it("should preserve optional styling properties", () => {
        const oldRectangle: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 200,
          width: 80,
          height: 60,
          fill: "#FF0000",
          stroke: "#000000",
          strokeWidth: 2,
        };

        const newRectangle: Extract<Shape, { type: "rect" }> = {
          ...oldRectangle,
          width: 120,
          height: 90,
        };

        shapesRef.current = [oldRectangle];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldRectangle,
          newRectangle,
        );

        command.execute();

        const resizedShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "rect" }
        >;
        expect(resizedShape.fill).toBe("#FF0000");
        expect(resizedShape.stroke).toBe("#000000");
        expect(resizedShape.strokeWidth).toBe(2);
      });
    });

    describe("Oval shapes", () => {
      it("should update oval dimensions in shapes array", () => {
        const oldOval: Extract<Shape, { type: "oval" }> = {
          id: "oval-1",
          type: "oval",
          x: 150,
          y: 250,
          width: 90,
          height: 70,
        };

        const newOval: Extract<Shape, { type: "oval" }> = {
          ...oldOval,
          width: 110,
          height: 85,
        };

        shapesRef.current = [oldOval];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldOval,
          newOval,
        );

        command.execute();

        expect(shapesRef.current).toHaveLength(1);
        const resizedShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "oval" }
        >;
        expect(resizedShape.width).toBe(110);
        expect(resizedShape.height).toBe(85);
      });

      it("should emit updateShape event with correct payload for oval", () => {
        const oldOval: Extract<Shape, { type: "oval" }> = {
          id: "oval-1",
          type: "oval",
          x: 150,
          y: 250,
          width: 90,
          height: 70,
        };

        const newOval: Extract<Shape, { type: "oval" }> = {
          ...oldOval,
          width: 110,
          height: 85,
        };

        shapesRef.current = [oldOval];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldOval,
          newOval,
        );

        command.execute();

        expect(mockSocket.emit).toHaveBeenCalledWith("updateShape", {
          boardId: boardId,
          shape: expect.objectContaining({
            id: "oval-1",
            type: "oval",
            width: 110,
            height: 85,
          }),
        });
      });

      it("should preserve oval position (x, y)", () => {
        const oldOval: Extract<Shape, { type: "oval" }> = {
          id: "oval-1",
          type: "oval",
          x: 150,
          y: 250,
          width: 90,
          height: 70,
        };

        const newOval: Extract<Shape, { type: "oval" }> = {
          ...oldOval,
          width: 120,
          height: 100,
        };

        shapesRef.current = [oldOval];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldOval,
          newOval,
        );

        command.execute();

        const resizedShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "oval" }
        >;
        expect(resizedShape.x).toBe(150);
        expect(resizedShape.y).toBe(250);
      });
    });

    describe("Socket event emission", () => {
      it("should emit updateShape event exactly once", () => {
        const oldShape: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 100,
          width: 50,
          height: 50,
        };

        const newShape: Extract<Shape, { type: "rect" }> = {
          ...oldShape,
          width: 80,
          height: 80,
        };

        shapesRef.current = [oldShape];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldShape,
          newShape,
        );

        command.execute();

        expect(mockSocket.emit).toHaveBeenCalledTimes(1);
        expect(mockSocket.emit).toHaveBeenCalledWith(
          "updateShape",
          expect.any(Object),
        );
      });

      it("should include boardId in socket payload", () => {
        const oldShape: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 100,
          width: 50,
          height: 50,
        };

        const newShape: Extract<Shape, { type: "rect" }> = {
          ...oldShape,
          width: 80,
          height: 80,
        };

        shapesRef.current = [oldShape];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldShape,
          newShape,
        );

        command.execute();

        expect(mockSocket.emit).toHaveBeenCalledWith(
          "updateShape",
          expect.objectContaining({
            boardId: boardId,
          }),
        );
      });

      it("should include shape in socket payload", () => {
        const oldShape: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 100,
          width: 50,
          height: 50,
        };

        const newShape: Extract<Shape, { type: "rect" }> = {
          ...oldShape,
          width: 80,
          height: 80,
        };

        shapesRef.current = [oldShape];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldShape,
          newShape,
        );

        command.execute();

        expect(mockSocket.emit).toHaveBeenCalledWith(
          "updateShape",
          expect.objectContaining({
            shape: expect.objectContaining({
              id: "rect-1",
              width: 80,
              height: 80,
            }),
          }),
        );
      });
    });

    describe("Multiple shapes in array", () => {
      it("should only update the target shape", () => {
        const shape1: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 100,
          width: 50,
          height: 50,
        };

        const shape2: Extract<Shape, { type: "rect" }> = {
          id: "rect-2",
          type: "rect",
          x: 200,
          y: 200,
          width: 60,
          height: 60,
        };

        const shape3: Extract<Shape, { type: "oval" }> = {
          id: "oval-1",
          type: "oval",
          x: 300,
          y: 300,
          width: 70,
          height: 70,
        };

        const newShape2: Extract<Shape, { type: "rect" }> = {
          ...shape2,
          width: 80,
          height: 80,
        };

        shapesRef.current = [shape1, shape2, shape3];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          shape2,
          newShape2,
        );

        command.execute();

        expect(shapesRef.current).toHaveLength(3);
        expect(shapesRef.current[0]).toEqual(shape1);
        const resizedShape = shapesRef.current[1] as Extract<
          Shape,
          { type: "rect" }
        >;
        expect(resizedShape.width).toBe(80);
        expect(resizedShape.height).toBe(80);
        expect(shapesRef.current[2]).toEqual(shape3);
      });
    });

    describe("Edge cases", () => {
      it("should handle resizing shape that doesn't exist in array", () => {
        const oldShape: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 100,
          width: 50,
          height: 50,
        };

        const newShape: Extract<Shape, { type: "rect" }> = {
          ...oldShape,
          width: 80,
          height: 80,
        };

        shapesRef.current = [];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldShape,
          newShape,
        );

        expect(() => command.execute()).not.toThrow();
        expect(mockSocket.emit).toHaveBeenCalledWith("updateShape", {
          boardId: boardId,
          shape: newShape,
        });
      });
    });
  });

  describe("undo", () => {
    describe("Rectangle shapes", () => {
      it("should restore rectangle to original dimensions", () => {
        const oldRectangle: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 200,
          width: 80,
          height: 60,
        };

        const newRectangle: Extract<Shape, { type: "rect" }> = {
          ...oldRectangle,
          width: 120,
          height: 90,
        };

        shapesRef.current = [oldRectangle];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldRectangle,
          newRectangle,
        );

        command.execute();
        const resizedShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "rect" }
        >;
        expect(resizedShape.width).toBe(120);
        expect(resizedShape.height).toBe(90);

        mockSocket.emit.mockClear();
        command.undo();

        const restoredShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "rect" }
        >;
        expect(restoredShape.width).toBe(80);
        expect(restoredShape.height).toBe(60);
      });

      it("should emit updateShape event with old dimensions", () => {
        const oldRectangle: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 200,
          width: 80,
          height: 60,
        };

        const newRectangle: Extract<Shape, { type: "rect" }> = {
          ...oldRectangle,
          width: 120,
          height: 90,
        };

        shapesRef.current = [oldRectangle];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldRectangle,
          newRectangle,
        );

        command.execute();
        mockSocket.emit.mockClear();
        command.undo();

        expect(mockSocket.emit).toHaveBeenCalledWith("updateShape", {
          boardId: boardId,
          shape: expect.objectContaining({
            id: "rect-1",
            type: "rect",
            width: 80,
            height: 60,
          }),
        });
      });

      it("should preserve all rectangle properties during undo", () => {
        const oldRectangle: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 200,
          width: 80,
          height: 60,
          fill: "#FF0000",
          stroke: "#000000",
          strokeWidth: 2,
        };

        const newRectangle: Extract<Shape, { type: "rect" }> = {
          ...oldRectangle,
          width: 120,
          height: 90,
        };

        shapesRef.current = [oldRectangle];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldRectangle,
          newRectangle,
        );

        command.execute();
        command.undo();

        const restoredShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "rect" }
        >;
        expect(restoredShape).toEqual(oldRectangle);
        expect(restoredShape.fill).toBe("#FF0000");
        expect(restoredShape.stroke).toBe("#000000");
        expect(restoredShape.strokeWidth).toBe(2);
      });
    });

    describe("Oval shapes", () => {
      it("should restore oval to original dimensions", () => {
        const oldOval: Extract<Shape, { type: "oval" }> = {
          id: "oval-1",
          type: "oval",
          x: 150,
          y: 250,
          width: 90,
          height: 70,
        };

        const newOval: Extract<Shape, { type: "oval" }> = {
          ...oldOval,
          width: 110,
          height: 85,
        };

        shapesRef.current = [oldOval];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldOval,
          newOval,
        );

        command.execute();
        const resizedShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "oval" }
        >;
        expect(resizedShape.width).toBe(110);
        expect(resizedShape.height).toBe(85);

        mockSocket.emit.mockClear();
        command.undo();

        const restoredShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "oval" }
        >;
        expect(restoredShape.width).toBe(90);
        expect(restoredShape.height).toBe(70);
      });

      it("should emit updateShape event with old dimensions for oval", () => {
        const oldOval: Extract<Shape, { type: "oval" }> = {
          id: "oval-1",
          type: "oval",
          x: 150,
          y: 250,
          width: 90,
          height: 70,
        };

        const newOval: Extract<Shape, { type: "oval" }> = {
          ...oldOval,
          width: 110,
          height: 85,
        };

        shapesRef.current = [oldOval];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldOval,
          newOval,
        );

        command.execute();
        mockSocket.emit.mockClear();
        command.undo();

        expect(mockSocket.emit).toHaveBeenCalledWith("updateShape", {
          boardId: boardId,
          shape: expect.objectContaining({
            id: "oval-1",
            type: "oval",
            width: 90,
            height: 70,
          }),
        });
      });
    });

    describe("Socket event emission", () => {
      it("should emit updateShape event exactly once on undo", () => {
        const oldShape: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 100,
          width: 50,
          height: 50,
        };

        const newShape: Extract<Shape, { type: "rect" }> = {
          ...oldShape,
          width: 80,
          height: 80,
        };

        shapesRef.current = [oldShape];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldShape,
          newShape,
        );

        command.execute();
        mockSocket.emit.mockClear();
        command.undo();

        expect(mockSocket.emit).toHaveBeenCalledTimes(1);
        expect(mockSocket.emit).toHaveBeenCalledWith(
          "updateShape",
          expect.any(Object),
        );
      });
    });

    describe("Multiple undo operations", () => {
      it("should handle multiple undo calls correctly", () => {
        const oldShape: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 100,
          width: 50,
          height: 50,
        };

        const newShape: Extract<Shape, { type: "rect" }> = {
          ...oldShape,
          width: 80,
          height: 80,
        };

        shapesRef.current = [oldShape];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldShape,
          newShape,
        );

        command.execute();
        command.undo();
        command.undo(); // Second undo

        // Shape should still be at original dimensions
        const shape = shapesRef.current[0] as Extract<Shape, { type: "rect" }>;
        expect(shape.width).toBe(50);
        expect(shape.height).toBe(50);
      });
    });

    describe("Undo with other shapes present", () => {
      it("should restore shape dimensions while preserving other shapes", () => {
        const shape1: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 100,
          width: 50,
          height: 50,
        };

        const shape2: Extract<Shape, { type: "rect" }> = {
          id: "rect-2",
          type: "rect",
          x: 200,
          y: 200,
          width: 60,
          height: 60,
        };

        const shape3: Extract<Shape, { type: "oval" }> = {
          id: "oval-1",
          type: "oval",
          x: 300,
          y: 300,
          width: 70,
          height: 70,
        };

        const newShape2: Extract<Shape, { type: "rect" }> = {
          ...shape2,
          width: 80,
          height: 80,
        };

        shapesRef.current = [shape1, shape2, shape3];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          shape2,
          newShape2,
        );

        command.execute();
        const resizedShape = shapesRef.current[1] as Extract<
          Shape,
          { type: "rect" }
        >;
        expect(resizedShape.width).toBe(80);
        expect(resizedShape.height).toBe(80);

        command.undo();

        expect(shapesRef.current).toHaveLength(3);
        expect(shapesRef.current[0]).toEqual(shape1);
        expect(shapesRef.current[1]).toEqual(shape2);
        expect(shapesRef.current[2]).toEqual(shape3);
      });
    });
  });

  describe("Execute-Undo round-trip", () => {
    it("should restore exact state after execute-undo for rectangle", () => {
      const oldRectangle: Extract<Shape, { type: "rect" }> = {
        id: "rect-1",
        type: "rect",
        x: 100,
        y: 200,
        width: 80,
        height: 60,
        fill: "#FF0000",
        stroke: "#000000",
        strokeWidth: 2,
      };

      const newRectangle: Extract<Shape, { type: "rect" }> = {
        ...oldRectangle,
        width: 120,
        height: 90,
      };

      shapesRef.current = [oldRectangle];
      const initialState = JSON.parse(JSON.stringify(shapesRef.current));

      const command = new ResizeShapeCommand(
        shapesRef,
        boardId,
        mockSocket as unknown as Socket,
        oldRectangle,
        newRectangle,
      );

      command.execute();
      command.undo();

      expect(shapesRef.current).toEqual(initialState);
    });

    it("should restore exact state after execute-undo for oval", () => {
      const oldOval: Extract<Shape, { type: "oval" }> = {
        id: "oval-1",
        type: "oval",
        x: 150,
        y: 250,
        width: 90,
        height: 70,
        fill: "#00FF00",
      };

      const newOval: Extract<Shape, { type: "oval" }> = {
        ...oldOval,
        width: 110,
        height: 85,
      };

      shapesRef.current = [oldOval];
      const initialState = JSON.parse(JSON.stringify(shapesRef.current));

      const command = new ResizeShapeCommand(
        shapesRef,
        boardId,
        mockSocket as unknown as Socket,
        oldOval,
        newOval,
      );

      command.execute();
      command.undo();

      expect(shapesRef.current).toEqual(initialState);
    });
  });

  describe("Property-Based Tests", () => {
    describe("Property 21: ResizeShapeCommand Execution Updates Dimensions", () => {
      // Feature: canvas-engine-testing, Property 21: ResizeShapeCommand Execution Updates Dimensions
      it("should update shape dimensions while preserving position and other properties", () => {
        fc.assert(
          fc.property(
            fc.oneof(arbRectangle, arbOval),
            arbDimension,
            arbDimension,
            (shape, newWidth, newHeight) => {
              // Setup
              mockSocket = createMockSocket();
              shapesRef = createMockShapesRef([shape]);

              // Create new shape with updated dimensions
              const newShape = { ...shape, width: newWidth, height: newHeight };

              // Create and execute command
              const command = new ResizeShapeCommand(
                shapesRef,
                boardId,
                mockSocket as unknown as Socket,
                shape,
                newShape,
              );

              command.execute();

              // Verify dimensions are updated
              const updatedShape = shapesRef.current[0];
              if (
                updatedShape.type === "rect" ||
                updatedShape.type === "oval"
              ) {
                expect(updatedShape.width).toBe(newWidth);
                expect(updatedShape.height).toBe(newHeight);

                // Verify position unchanged
                expect(updatedShape.x).toBe(shape.x);
                expect(updatedShape.y).toBe(shape.y);
              }

              // Verify type and id unchanged
              expect(updatedShape.type).toBe(shape.type);
              expect(updatedShape.id).toBe(shape.id);

              // Verify socket emits updateShape event
              expect(mockSocket.emit).toHaveBeenCalledWith(
                "updateShape",
                expect.objectContaining({
                  boardId: boardId,
                  shape: expect.objectContaining({
                    id: shape.id,
                    type: shape.type,
                    width: newWidth,
                    height: newHeight,
                  }),
                }),
              );
            },
          ),
          { numRuns: 100 },
        );
      });

      // Feature: canvas-engine-testing, Property 21: ResizeShapeCommand Execution Updates Dimensions
      it("should update dimensions for all resizable shape types (rectangles, ovals)", () => {
        fc.assert(
          fc.property(
            fc.oneof(arbRectangle, arbOval),
            arbDimension,
            arbDimension,
            (shape, newWidth, newHeight) => {
              // Setup
              mockSocket = createMockSocket();
              shapesRef = createMockShapesRef([shape]);

              // Create new shape with updated dimensions
              const newShape = { ...shape, width: newWidth, height: newHeight };

              // Create and execute command
              const command = new ResizeShapeCommand(
                shapesRef,
                boardId,
                mockSocket as unknown as Socket,
                shape,
                newShape,
              );

              command.execute();

              // Verify shape is updated in array
              expect(shapesRef.current).toHaveLength(1);
              const updatedShape = shapesRef.current[0];

              // Verify dimensions are updated
              if (
                updatedShape.type === "rect" ||
                updatedShape.type === "oval"
              ) {
                expect(updatedShape.width).toBe(newWidth);
                expect(updatedShape.height).toBe(newHeight);
              }

              // Verify socket event
              expect(mockSocket.emit).toHaveBeenCalledWith(
                "updateShape",
                expect.any(Object),
              );
            },
          ),
          { numRuns: 100 },
        );
      });
    });

    describe("Property 22: ResizeShapeCommand Round-Trip", () => {
      // Feature: canvas-engine-testing, Property 22: ResizeShapeCommand Round-Trip
      it("should restore exact original dimensions after execute-undo", () => {
        fc.assert(
          fc.property(
            fc.oneof(arbRectangle, arbOval),
            arbDimension,
            arbDimension,
            (shape, newWidth, newHeight) => {
              // Setup
              mockSocket = createMockSocket();
              shapesRef = createMockShapesRef([shape]);
              const initialState = JSON.parse(
                JSON.stringify(shapesRef.current),
              );

              // Create new shape with updated dimensions
              const newShape = { ...shape, width: newWidth, height: newHeight };

              // Create and execute command
              const command = new ResizeShapeCommand(
                shapesRef,
                boardId,
                mockSocket as unknown as Socket,
                shape,
                newShape,
              );

              command.execute();

              // Clear socket emit calls from execute
              mockSocket.emit.mockClear();

              // Undo
              command.undo();

              // Verify exact state restoration
              expect(shapesRef.current).toEqual(initialState);

              // Verify all properties unchanged
              const restoredShape = shapesRef.current[0];
              expect(restoredShape).toEqual(shape);
              expect(restoredShape.id).toBe(shape.id);
              expect(restoredShape.type).toBe(shape.type);

              // Verify socket emits updateShape event with old dimensions
              expect(mockSocket.emit).toHaveBeenCalledWith(
                "updateShape",
                expect.objectContaining({
                  boardId: boardId,
                  shape: expect.objectContaining({
                    id: shape.id,
                    type: shape.type,
                  }),
                }),
              );
            },
          ),
          { numRuns: 100 },
        );
      });

      // Feature: canvas-engine-testing, Property 22: ResizeShapeCommand Round-Trip
      it("should handle round-trip for all resizable shape types", () => {
        fc.assert(
          fc.property(
            fc.oneof(arbRectangle, arbOval),
            arbDimension,
            arbDimension,
            (shape, newWidth, newHeight) => {
              // Setup
              mockSocket = createMockSocket();
              shapesRef = createMockShapesRef([shape]);

              // Create new shape with updated dimensions
              const newShape = { ...shape, width: newWidth, height: newHeight };

              // Create and execute command
              const command = new ResizeShapeCommand(
                shapesRef,
                boardId,
                mockSocket as unknown as Socket,
                shape,
                newShape,
              );

              command.execute();
              command.undo();

              // Verify shape is restored to original state
              expect(shapesRef.current).toHaveLength(1);
              expect(shapesRef.current[0]).toEqual(shape);
            },
          ),
          { numRuns: 100 },
        );
      });

      // Feature: canvas-engine-testing, Property 22: ResizeShapeCommand Round-Trip
      it("should preserve shape in array with other shapes during round-trip", () => {
        fc.assert(
          fc.property(
            fc.oneof(arbRectangle, arbOval),
            fc.array(arbShape, { minLength: 0, maxLength: 10 }),
            arbDimension,
            arbDimension,
            (shapeToResize, otherShapes, newWidth, newHeight) => {
              // Ensure shapeToResize has unique ID
              const uniqueOtherShapes = otherShapes.filter(
                (s) => s.id !== shapeToResize.id,
              );

              // Setup
              mockSocket = createMockSocket();
              shapesRef = createMockShapesRef([
                ...uniqueOtherShapes,
                shapeToResize,
              ]);
              const initialLength = shapesRef.current.length;

              // Create new shape with updated dimensions
              const newShape = {
                ...shapeToResize,
                width: newWidth,
                height: newHeight,
              };

              // Create and execute command
              const command = new ResizeShapeCommand(
                shapesRef,
                boardId,
                mockSocket as unknown as Socket,
                shapeToResize,
                newShape,
              );

              command.execute();
              command.undo();

              // Verify shape count is unchanged
              expect(shapesRef.current).toHaveLength(initialLength);

              // Verify resized shape is restored
              const restoredShape = shapesRef.current.find(
                (s) => s.id === shapeToResize.id,
              );
              expect(restoredShape).toEqual(shapeToResize);

              // Verify other shapes are unchanged
              uniqueOtherShapes.forEach((otherShape) => {
                expect(shapesRef.current).toContainEqual(otherShape);
              });
            },
          ),
          { numRuns: 100 },
        );
      });
    });
  });

  describe("Shape property invariants", () => {
    describe("Shape ID remains unchanged through resize operations", () => {
      it("should preserve shape ID when resizing rectangle", () => {
        const oldRectangle: Extract<Shape, { type: "rect" }> = {
          id: "rect-unique-id-123",
          type: "rect",
          x: 100,
          y: 200,
          width: 80,
          height: 60,
        };

        const newRectangle: Extract<Shape, { type: "rect" }> = {
          ...oldRectangle,
          width: 120,
          height: 90,
        };

        shapesRef.current = [oldRectangle];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldRectangle,
          newRectangle,
        );

        command.execute();

        const resizedShape = shapesRef.current[0];
        expect(resizedShape.id).toBe("rect-unique-id-123");
        expect(resizedShape.id).toBe(oldRectangle.id);
      });

      it("should preserve shape ID when resizing oval", () => {
        const oldOval: Extract<Shape, { type: "oval" }> = {
          id: "oval-unique-id-456",
          type: "oval",
          x: 150,
          y: 250,
          width: 90,
          height: 70,
        };

        const newOval: Extract<Shape, { type: "oval" }> = {
          ...oldOval,
          width: 110,
          height: 85,
        };

        shapesRef.current = [oldOval];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldOval,
          newOval,
        );

        command.execute();

        const resizedShape = shapesRef.current[0];
        expect(resizedShape.id).toBe("oval-unique-id-456");
        expect(resizedShape.id).toBe(oldOval.id);
      });

      it("should preserve shape ID after undo", () => {
        const oldShape: Extract<Shape, { type: "rect" }> = {
          id: "rect-id-undo-test",
          type: "rect",
          x: 100,
          y: 100,
          width: 50,
          height: 50,
        };

        const newShape: Extract<Shape, { type: "rect" }> = {
          ...oldShape,
          width: 80,
          height: 80,
        };

        shapesRef.current = [oldShape];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldShape,
          newShape,
        );

        command.execute();
        command.undo();

        const restoredShape = shapesRef.current[0];
        expect(restoredShape.id).toBe("rect-id-undo-test");
        expect(restoredShape.id).toBe(oldShape.id);
      });
    });

    describe("Shape type remains unchanged through resize operations", () => {
      it("should preserve shape type when resizing rectangle", () => {
        const oldRectangle: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 200,
          width: 80,
          height: 60,
        };

        const newRectangle: Extract<Shape, { type: "rect" }> = {
          ...oldRectangle,
          width: 120,
          height: 90,
        };

        shapesRef.current = [oldRectangle];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldRectangle,
          newRectangle,
        );

        command.execute();

        const resizedShape = shapesRef.current[0];
        expect(resizedShape.type).toBe("rect");
        expect(resizedShape.type).toBe(oldRectangle.type);
      });

      it("should preserve shape type when resizing oval", () => {
        const oldOval: Extract<Shape, { type: "oval" }> = {
          id: "oval-1",
          type: "oval",
          x: 150,
          y: 250,
          width: 90,
          height: 70,
        };

        const newOval: Extract<Shape, { type: "oval" }> = {
          ...oldOval,
          width: 110,
          height: 85,
        };

        shapesRef.current = [oldOval];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldOval,
          newOval,
        );

        command.execute();

        const resizedShape = shapesRef.current[0];
        expect(resizedShape.type).toBe("oval");
        expect(resizedShape.type).toBe(oldOval.type);
      });

      it("should preserve shape type after undo", () => {
        const oldShape: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 100,
          width: 50,
          height: 50,
        };

        const newShape: Extract<Shape, { type: "rect" }> = {
          ...oldShape,
          width: 80,
          height: 80,
        };

        shapesRef.current = [oldShape];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldShape,
          newShape,
        );

        command.execute();
        command.undo();

        const restoredShape = shapesRef.current[0];
        expect(restoredShape.type).toBe("rect");
        expect(restoredShape.type).toBe(oldShape.type);
      });
    });

    describe("Resize preserves position", () => {
      it("should preserve x and y when resizing rectangle", () => {
        const oldRectangle: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 200,
          width: 80,
          height: 60,
        };

        const newRectangle: Extract<Shape, { type: "rect" }> = {
          ...oldRectangle,
          width: 150,
          height: 120,
        };

        shapesRef.current = [oldRectangle];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldRectangle,
          newRectangle,
        );

        command.execute();

        const resizedShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "rect" }
        >;
        expect(resizedShape.x).toBe(100);
        expect(resizedShape.y).toBe(200);
        expect(resizedShape.x).toBe(oldRectangle.x);
        expect(resizedShape.y).toBe(oldRectangle.y);
      });

      it("should preserve x and y when resizing oval", () => {
        const oldOval: Extract<Shape, { type: "oval" }> = {
          id: "oval-1",
          type: "oval",
          x: 150,
          y: 250,
          width: 90,
          height: 70,
        };

        const newOval: Extract<Shape, { type: "oval" }> = {
          ...oldOval,
          width: 120,
          height: 100,
        };

        shapesRef.current = [oldOval];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldOval,
          newOval,
        );

        command.execute();

        const resizedShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "oval" }
        >;
        expect(resizedShape.x).toBe(150);
        expect(resizedShape.y).toBe(250);
        expect(resizedShape.x).toBe(oldOval.x);
        expect(resizedShape.y).toBe(oldOval.y);
      });

      it("should preserve position after undo", () => {
        const oldShape: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 100,
          width: 50,
          height: 50,
        };

        const newShape: Extract<Shape, { type: "rect" }> = {
          ...oldShape,
          width: 80,
          height: 80,
        };

        shapesRef.current = [oldShape];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldShape,
          newShape,
        );

        command.execute();
        command.undo();

        const restoredShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "rect" }
        >;
        expect(restoredShape.x).toBe(100);
        expect(restoredShape.y).toBe(100);
        expect(restoredShape.x).toBe(oldShape.x);
        expect(restoredShape.y).toBe(oldShape.y);
      });
    });

    describe("Undo restores all properties exactly", () => {
      it("should restore all rectangle properties exactly after undo", () => {
        const oldRectangle: Extract<Shape, { type: "rect" }> = {
          id: "rect-1",
          type: "rect",
          x: 100,
          y: 200,
          width: 80,
          height: 60,
          fill: "#FF0000",
          stroke: "#000000",
          strokeWidth: 2,
        };

        const newRectangle: Extract<Shape, { type: "rect" }> = {
          ...oldRectangle,
          width: 120,
          height: 90,
        };

        shapesRef.current = [oldRectangle];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldRectangle,
          newRectangle,
        );

        command.execute();
        command.undo();

        const restoredShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "rect" }
        >;
        expect(restoredShape).toEqual(oldRectangle);
        expect(restoredShape.id).toBe(oldRectangle.id);
        expect(restoredShape.type).toBe(oldRectangle.type);
        expect(restoredShape.x).toBe(oldRectangle.x);
        expect(restoredShape.y).toBe(oldRectangle.y);
        expect(restoredShape.width).toBe(oldRectangle.width);
        expect(restoredShape.height).toBe(oldRectangle.height);
        expect(restoredShape.fill).toBe(oldRectangle.fill);
        expect(restoredShape.stroke).toBe(oldRectangle.stroke);
        expect(restoredShape.strokeWidth).toBe(oldRectangle.strokeWidth);
      });

      it("should restore all oval properties exactly after undo", () => {
        const oldOval: Extract<Shape, { type: "oval" }> = {
          id: "oval-1",
          type: "oval",
          x: 150,
          y: 250,
          width: 90,
          height: 70,
          fill: "#00FF00",
        };

        const newOval: Extract<Shape, { type: "oval" }> = {
          ...oldOval,
          width: 110,
          height: 85,
        };

        shapesRef.current = [oldOval];

        const command = new ResizeShapeCommand(
          shapesRef,
          boardId,
          mockSocket as unknown as Socket,
          oldOval,
          newOval,
        );

        command.execute();
        command.undo();

        const restoredShape = shapesRef.current[0] as Extract<
          Shape,
          { type: "oval" }
        >;
        expect(restoredShape).toEqual(oldOval);
        expect(restoredShape.id).toBe(oldOval.id);
        expect(restoredShape.type).toBe(oldOval.type);
        expect(restoredShape.x).toBe(oldOval.x);
        expect(restoredShape.y).toBe(oldOval.y);
        expect(restoredShape.width).toBe(oldOval.width);
        expect(restoredShape.height).toBe(oldOval.height);
        expect(restoredShape.fill).toBe(oldOval.fill);
      });
    });
  });
});
